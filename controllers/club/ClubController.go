...
func (club ClubController) DeleteMember(c *gin.Context) { //删除球员完成
	...
	name := c.Query("name")
	if id == "" && name == "" {
		if err := models.DB.Delete(&models.Member{}).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete all members"})
			return
		}
		c.JSON(http.StatusOK, gin.H{"message": "All members deleted successfully"})
		return
	}
	if id != "" {
